...
void bubble_sort(void*  data,
                 ...
                 SwapIf swap_if) {
    if(NULL == data || 0 == bytes_per_element || 2 > element_count) { return; }
    ...
    for(size_t left = 1; left < element_count; ++left) {
        ...
        for(size_t right = element_count - 1; right >= left; --right) {
            if(swap_if((uint8_t*)data + bytes_per_element * (right - 1),
                       (uint8_t*)data + bytes_per_element * right)) {
                swap(ex_buff,
                     (uint8_t*)data + bytes_per_element * (right - 1),
                     (uint8_t*)data + bytes_per_element * right,
                     bytes_per_element);
                sorted = false;
            }
...
void insert_sort(void*  data,
    ...
    for(size_t right = 1; right < element_count; ++right) {
        ...
        for(;
            left > 0 && swap_if((uint8_t*)data + bytes_per_element * (left - 1),
                                marked_value);
            --left) {
            memcpy((uint8_t*)data + bytes_per_element * left,
                   (uint8_t*)data + bytes_per_element * (left - 1),
                   bytes_per_element);
        }
...
void selection_sort(void*  data,
    ...
    for(size_t left = 1; left < element_count; ++left) {
        ...
        for(size_t right = left; right < element_count; ++right) {
            if(swap_if((uint8_t*)data + bytes_per_element * marked_index,
                       (uint8_t*)data + bytes_per_element * right)) {
                marked_index = right;
            }
        }
        if(left - 1 != marked_index)
            swap(ex_buff,
                 (uint8_t*)data + bytes_per_element * (left - 1),
                 (uint8_t*)data + bytes_per_element * marked_index,
                 bytes_per_element);
    }
...
void quick_sort(void*  data,
    ...
    swap(ex_buff,
         (uint8_t*)data + bytes_per_element * 1,
         (uint8_t*)data + bytes_per_element * (element_count / 2),
         bytes_per_element);
    swap(ex_buff,
         (uint8_t*)data + bytes_per_element * 2,
         (uint8_t*)data + bytes_per_element * (element_count - 1),
         bytes_per_element);
    ...
    while(left <= right) {
        while(left <= right) {
            if(swap_if((uint8_t*)data + bytes_per_element * left,
                       (uint8_t*)data + bytes_per_element * 1)) {
                break;
            }
            ++left;
        ...
        while(left <= right) {
            if(swap_if((uint8_t*)data + bytes_per_element * 1,
                       (uint8_t*)data + bytes_per_element * right)) {
                break;
            }
            --right;
        ...
        if(left < right) {
            swap(ex_buff,
                 (uint8_t*)data + bytes_per_element * left,
                 (uint8_t*)data + bytes_per_element * right,
                 bytes_per_element);
            ++left;
            --right;
    ...
    quick_sort((uint8_t*)data + bytes_per_element * left,
               bytes_per_element,
               element_count - left,
               swap_if);
...
static void merge(void*  dest,
    ...
    for(; left_i < left_element_count && right_i < right_element_count;
        ++dest_i) {
        if(swap_if((uint8_t*)left + bytes_per_element * left_i,
                   (uint8_t*)right + bytes_per_element * right_i)) {

            memcpy((uint8_t*)dest + bytes_per_element * dest_i,
                   (uint8_t*)right + bytes_per_element * right_i,
                   bytes_per_element);
            ++right_i;
        } else {
            memcpy((uint8_t*)dest + bytes_per_element * dest_i,
                   (uint8_t*)left + bytes_per_element * left_i,
                   bytes_per_element);
            ++left_i;
        }
    }
...
static void merge_sort_pri(void*  data,
    ...
    merge_sort_pri((uint8_t*)data + bytes_per_element * middle,
                   (uint8_t*)temp + bytes_per_element * middle,
                   bytes_per_element,
    ...
    merge(temp,
          data,
          middle,
          (uint8_t*)data + bytes_per_element * middle,
          element_count - middle,
          bytes_per_element,
          swap_if);
